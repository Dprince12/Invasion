...
// time to move ppl out of the helo;
		waitUntil {(getpos _helo select 2 < 4 && _helo distance helipad0<20)  or (vehicle _pilot == _pilot or !alive _pilot)}; // wait until the helo is near the ground
// IF THE PILOT IS DEAD OR CHOPPA DOWN  **************
		[_pilot] call LF_fnc_deadCheck;
		if (crashed) Exitwith {
		...
		_helo vehicleChat format["We've arrived at the Base. %1. %2",_plName, _phrasing];
		_helo lock false;
